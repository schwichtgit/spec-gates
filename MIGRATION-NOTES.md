...
| runtime/hooks/claude/*.sh                 | CPF hooks               | DONE: verify-quality.sh slimmed 617->60 lines, now delegates to verify.sh --boundary agent (legacy language walk dropped); PreToolUse/format hooks reviewed: fixed validate-bash `rm -rf /` (GNU-grep \b), and a path bug where post-edit + format-changed resolved the lib as $SCRIPT_DIR/../lib (nonexistent) so auto-format silently no-op'd -- now resolve .specify/gates/lib and are covered by real formatting tests; check-upgrade.sh was intentionally dropped |
